<?php
 ...
 $idOrdem = trim($_POST["idOrdem"]);
  ...
  $resultOrdem2 = mysqli_query($_SG['link'], $sql);
    
    
   $dadosOrdem2 = mysqli_fetch_array($resultOrdem2);

  if ($dadosOrdem2 == null){
       echo "<script>alert('ERROOOO');window.location.href='ordemServico.php';</script>";
      }

   $idOrdem = $dadosOrdem2['idOrdem'];
   $status = $dadosOrdem2['statusOrdem'];
   $valorTotal = $dadosOrdem2['valorTotal'];
   $valorMO = $dadosOrdem2['valorMO'];
   $valorPecas = $dadosOrdem2['valorPecas'];
   $descricao = $dadosOrdem2['descricaoServico'];
   $cpf = $dadosOrdem2['cpfCliente'];
